...
describe('ERC1077', () => {
  ...
  beforeAll(async () => {
    wallet = Wallet.fromMnemonic(
      'library upon horse concert horse crunch copy dice flash design drastic cushion'
    );
    wallet = wallet.connect(getDefaultProvider('ropsten'));
    erc1077 = new ERC1077('harrypotter', wallet);
  });
  ...
  test('send() should throw (invalid signature)', async () => {
    ...
    const otherErc1077 = new ERC1077('harrypotter', otherWallet);
    otherErc1077
      .send(tx)
