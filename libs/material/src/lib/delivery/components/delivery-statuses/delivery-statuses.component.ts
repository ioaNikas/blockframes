...
import { DeliveryQuery, DeliveryService, DeliveryStatus, MGDeadline } from '../../+state';
...
export class DeliveryStatusesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.currentStatus$ = combineLatest([this.query.statuses$, this.query.currentStatus$]).pipe(
